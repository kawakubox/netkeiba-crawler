class ChangePkOnHorses < ActiveRecord::Migration[5.0]
  def up
    # Drop materialized view
    execute('DROP MATERIALIZED VIEW race_entries')

    # horse_results.horse_id 型変更
    change_column :horse_results, :horse_id, :string

    # horse_results.horse_id 更新
    execute('UPDATE horse_results SET horse_id = horses.key FROM horses WHERE horse_id = CAST(horses.id AS VARCHAR(10))')

    # horses.id 型変更
    change_column :horses, :id, :string

    # horses.id を horses.key の値で更新
    execute('UPDATE horses SET id = "key"')
